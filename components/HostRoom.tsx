...
export const HostRoom: React.FC<HostRoomProps> = ({ onBack }) => { 
  ...
  // --- NEW: Handle video element reset when Stream URL changes ---
  useEffect(() => {
      if (fileVideoRef.current && fileStreamUrl && fileStreamUrl.startsWith('http')) {
          // When URL changes (seek), browser sees new stream.
          // We MUST force local time to 0 because the new stream starts at 0.
          // Otherwise browser tries to seek to 'old' timestamp in new stream and freezes.
          fileVideoRef.current.currentTime = 0;
          fileVideoRef.current.load(); // Explicit load call is safer
          
          const playPromise = fileVideoRef.current.play();
          if (playPromise !== undefined) {
            playPromise
              .then(() => setIsPlayingFile(true))
              .catch(e => console.log("Play interrupted or autoplay prevented", e));
          }
      }
  }, [fileStreamUrl]);
  // -------------------------------------------------------------
  ...
  const handleFileSeek = (e: React.ChangeEvent<HTMLInputElement>) => {  
      ...
      if (fileStreamUrl && fileStreamUrl.startsWith('http')) {
          if (fileRawPath) {
              ...
              // Update state to trigger re-render with new src
              // We add a timestamp to force the browser to treat it as a new resource
    ...
    <div className="flex h-screen bg-[#111] text-gray-100 overflow-hidden font-sans">  
      <div className="flex-1 flex flex-col relative min-w-0">  
        <div ref={containerRef} className="flex-1 relative bg-black flex items-center justify-center overflow-hidden group select-none"  
            ...
            <video ref={fileVideoRef} src={fileStreamUrl || ''} className="absolute top-0 left-0 w-full h-full -z-50 opacity-100 pointer-events-none" style={{ visibility: 'visible' }} playsInline crossOrigin="anonymous" onTimeUpdate={handleFileTimeUpdate}>  
